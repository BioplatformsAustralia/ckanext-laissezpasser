...
def laissezpasser_package_passes(user=None, filter_valid=False):
    ...
    passes_as_dicts = [
        model_dictize.package_dictize(
            model.Session.query(model.Package).get(pkg_name), context
        )
...
def laissezpasser_held_passes(user=None, filter_valid=False):
    ...
    held_as_dicts = [
        d.table_dictize(
            LaissezpasserPassesTable.get_by_package_and_user(
                metadata.dataset, metadata.user_name
            )[0],
            context,
        )
    ...
    held_as_dicts = [
        dict(held, **{"valid": l.valid(held.get("dataset"), held.get("user_name"))})
        for held in held_as_dicts
    ]
...
def laissezpasser_issued_passes(package=None, filter_valid=False):
    ...
    issued_as_dicts = [
        dict(
            issued, **{"valid": l.valid(issued.get("dataset"), issued.get("user_name"))}
        )
        for issued in issued_as_dicts
    ]
