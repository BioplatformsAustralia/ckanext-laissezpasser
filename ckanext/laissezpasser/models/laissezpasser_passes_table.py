...
laissezpasser_passes_table = Table(
    ...
    Column(
        u"created_at", _types.DateTime, default=datetime.datetime.utcnow, nullable=False
    ),
...
class LaissezpasserPassesTable(domain_object.DomainObject):
    ...
    def get_by_user(cls, name, autoflush=True):
        ...
        exists = (
            meta.Session.query(cls).filter(cls.user_name == name).first() is not None
        )
        if not exists:
            return False
    ...
    def get_by_package_and_user(cls, dataset, user, autoflush=True):
        ...
        exists = (
            ...
            .filter(cls.dataset == dataset, cls.user_name == user)
            .first()
            is not None
        )
        if not exists:
        ...
        query = meta.Session.query(cls).filter(
            cls.dataset == dataset, cls.user_name == user
        )
        query = query.autoflush(autoflush)
        record = query.all()
        return record
...
meta.mapper(
    ...
    properties={
        u"package": orm.relation(
            Package,
            backref=orm.backref(
                u"laissezpasser_passes_table", cascade=u"all, delete, delete-orphan"
            ),
        ),
        u"user": orm.relation(
            User,
